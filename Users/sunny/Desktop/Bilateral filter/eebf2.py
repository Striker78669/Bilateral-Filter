...
class ImageProcessor:
    ...
    def apply_bilateral_filter(self, sigma_color, sigma_space, sigma_entropy):
        ...
        # Modify sigma_color and sigma_space based on entropy
        adjusted_sigma_color = sigma_color + sigma_entropy * entropy
        adjusted_sigma_space = sigma_space + sigma_entropy * entropy

        # Apply bilateral filter with entropy as an additional factor
        filtered_area = cv2.bilateralFilter(selected_region, 9, adjusted_sigma_color, adjusted_sigma_space)
    ...
    def calculate_entropy(self, image):
        ...
        entropy = -np.sum(non_zero_bins * np.log2(non_zero_bins)) #shannon entropy formula
        return entropy
